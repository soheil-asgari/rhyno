...
export default function ChatUploadPage({
  ...
  const autoSaveToDatabase = async (fileUrl: string, rawFileName: string) => {
    ...
    const { error } = await supabase.from("payment_requests").insert({
      ...
      // 👇 نام داینامیک از روی فایل 👇
      supplier_name: dynamicName,

      // توضیحات را هم دقیقتر میکنیم
      description: `آپلود اولیه: ${rawFileName}`,

      amount: 0,
      status: "uploaded", // وضعیت اولیه
  ...
  const processImage = async (file: File, msgId: string) => {
    ...
    // ج) ذخیره فوری در دیتابیس (قسمت جدید) 👈
    ...
    // د) شروع آنالیز (اختیاری - اگر میخواهید آنالیز هم بشود بماند)
  ...
  const processPdf = async (file: File, msgId: string) => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      // ج) ذخیره فوری هر صفحه در دیتابیس (قسمت جدید) 👈
  ...
  const handleConfirm = async (data: AIResult, fileUrls: string | string[]) => {
    const toastId = toast.loading("در حال ثبت اسناد در سیستم حسابداری...")
    ...
    const res = await submitGroupedTransactions(
      params.workspaceid,
      groupedPayload
    )
    toast.dismiss(toastId)
    if (res.success) {
      toast.success("تراکنش‌ها با موفقیت ثبت شدند ✅")
      setMessages(prev => [
        ...prev,
        {
          id: Date.now().toString(),
          role: "system",
          content: `✅ عملیات موفق: ${res.count} سند حسابداری صادر شد.`
        }
      ])
    } else {
      toast.error("خطا در ثبت اسناد")
    }
  }
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-gray-50 font-sans">
      ...
      <header className="fixed inset-x-0 top-0 z-50 flex items-center justify-between border-b border-gray-200 bg-white/80 p-4 shadow-sm backdrop-blur-xl md:px-6">
        ...
        {/* دکمه مشاهده گزارشات (ریسپانسیو شده) */}
        <Link href={`/enterprise/${params.workspaceid}/finance/documents`}>
          {/* نسخه دسکتاپ: دکمه کامل */}
          <Button
            variant="outline"
            size="sm"
                ...
                <div className="mr-11 w-full max-w-lg">
                  <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xl shadow-gray-200/50">
                    ...
                    <div className="border-t border-gray-100 bg-gray-50 p-3">
                      <Button
                        onClick={() => handleConfirm(msg.data!, msg.fileUrl!)}
                        className="h-10 w-full rounded-xl bg-[#1e293b] text-xs font-medium text-white shadow-lg shadow-gray-300/50 transition-transform hover:bg-black active:scale-95"
                      >
                        ثبت نهایی {msg.data.transactions.length} تراکنش در سیستم
                      </Button>
                    </div>
                  </div>
                </div>
      ...
      {/* --- نوار ابزار پایین (اصلاح شده کامل) --- */}
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 p-4 sm:p-6">
        ...
        {/* دکمه تست دیتابیس (قرمز) - اصلاح شد */}
        {/* <Button
                    variant="destructive"
                    // 👇 کلاس pointer-events-auto اضافه شد 👇
                    className="fixed bottom-24 left-4 z-50 shadow-xl pointer-events-auto"
                    onClick={async () => {
                        console.log("شروع تست ثبت دستی...");
                        const { data, error } = await supabase
                            .from('payment_requests')
                            .insert({
                                workspace_id: params.workspaceid,
                                supplier_name: "تست دستی سیستم",
                                amount: 50000,
                                description: "تست سلامت دیتابیس",
                                status: "pending_docs",
                                receipt_image_url: "https://placehold.co/600x400.png",
                                payment_date: new Date().toISOString().split('T')[0]
                            })
                            .select();
                        ...
                        if (error) {
                            ...
                            alert("✅ ثبت شد! حالا به صفحه گزارشات بروید.");
