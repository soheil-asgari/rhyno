...
export function generateCleanDescription(
  ...
  if (!rawDesc) return type === "deposit" ? "واریز وجه" : "برداشت وجه"

  // ۱. حذف عبارات سیستمی و نام فایل‌های آپلود شده
  let clean = rawDesc
    .replace(/آپلود اولیه:.*\.pdf/gi, "") // حذف نام فایل PDF
    .replace(/No Title/gi, "")
    .replace(/[۰-۹]+-[۰-۹]+-[۰-۹]+/g, "") // حذف تاریخ‌های فرمت شده اگر وسط متن باشند (اختیاری)
    .replace(/\s+/g, " ") // تبدیل چند اسپیس به یک اسپیس
    .trim()

  // ۲. نرمال‌سازی و حذف عبارات تکراری
  // جملات را با خط تیره (-)، ویرگول (،) یا خط جدید جدا می‌کنیم
  const parts = clean.split(/[-–_،,\n]/).map(p => p.trim())

  // استفاده از Set برای حذف جملات کاملاً تکراری
  // فیلتر کردن بخش‌های خیلی کوتاه (کمتر از ۲ حرف)
  const uniqueParts = [...new Set(parts)].filter(p => p.length > 2)

  // ۳. بازسازی متن نهایی
  let finalDesc = uniqueParts.join(" - ")

  // ۴. اگر متن خیلی کوتاه شد یا خالی شد، یک متن پیش‌فرض بساز
  if (!finalDesc || finalDesc.length < 3) {
    const action = type === "deposit" ? "واریز از" : "واریز به"
    // اگر طرف حساب مشخص است، اسمش را بگذار
